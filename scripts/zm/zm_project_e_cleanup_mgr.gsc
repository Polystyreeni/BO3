...
#namespace zm_project_e_cleanup;

#define TRACKING_INNER_DIST					2000
#define TRACKING_OUTER_DIST					2200
#define TOO_HIGH_DIST						800

#define N_CLEANUP_INTERVAL_MIN				4000	// Minimum time in msec for cleanup checks
...
REGISTER_SYSTEM_EX( "zm_project_e_cleanup", &__init__, &__main__, undefined )
...
function private delete_zombie_noone_looking()
{
	...
	if(IS_TRUE(self.missing_legs))	// If this guy is a crawler, just teleport him to a new location instead, so that players can save crawlers
	{
		/*player = array::random(level.players);
		zone = array::random(get_adjacencies_to_zone( player zm_utility::get_current_zone( false ) ));
		if(!isdefined(zone))
		{
			return;
		}
		...
		{
			if( loc zm_utility::get_current_zone( true ) == zone )
			{
				spawn_point = loc;
				continue;
			}
		...
		self.linkmodel = Spawn("script_model", self.origin);
		self.linkmodel EnableLinkTo();
		self EnableLinkTo();
		self LinkTo( self.linkmodel );
		self.linkmodel.origin = spawn_point.origin;
		wait(0.05);

		self Unlink();
		self.linkmodel Delete();
